...
def carmel(inputs,dictionary_epron, dictionary_e_toj):
    #use the findback to find the all result base on the inputs string
    ...
    len_strings = len(new) #total number 
...
#print dictionary_epron

list = file2.readlines()
#becasue pause: pause is hard to change 
...
for line in list:
    ...
    for s in strings[2:len(strings)-3]:
        tmp = tmp + s + " "
    #dictionary_e_toj[strings[0]][tmp] = strings[len(strings)-2]
    dictionary_e_toj[tmp][strings[0]]= float(strings[len(strings)-2] )# p( tmp|string[0])
#print dictionary_e_toj
